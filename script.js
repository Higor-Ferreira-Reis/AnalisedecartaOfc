  ...
  function renderCriterios() {
    ...
    headers.slice(0, -2).forEach(col => {
      ...
      // Botão de ativar/desativar critério
    ...
    // Evento dos botões de ativar/desativar
  ...
  function getCriteriosFromUI() {
    ...
    headers.slice(0, -2).forEach(col => {
      if (ignorar.includes(col)) return;
      // Só pega critérios se estiver ativo
  ...
  function aplicarCriterios() {
    ...
    // Conta quantos critérios estão ativos
    ...
    tableData.forEach((row, idx) => {
      ...
      // Lógica baseada na quantidade de critérios ativos
      let notaStr = '';
      ...
      <table class="min-w-full text-sm mb-6">
        <tbody>
          ${headers.slice(0, -2).map((col, i) => `
            <tr>
              <td class="font-semibold pr-2 py-1 text-blue-700">${col}</td>
              <td class="py-1">${row[i] !== undefined && row[i] !== null ? row[i] : ''}</td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      <div class="font-bold text-blue-700 mb-2">Notas por Critério</div>
      <table class="min-w-full text-xs border mb-2">
        <thead class="bg-blue-100">
          <tr>
            <th class="px-2 py-1">Coluna</th>
            <th class="px-2 py-1">Valor</th>
            <th class="px-2 py-1">Faixa</th>
            <th class="px-2 py-1">Nota</th>
            <th class="px-2 py-1">Cor</th>
          </tr>
        </thead>
        <tbody>
          ${(() => {
            const criterios = getCriteriosFromUI();
            return headers.slice(0, -2).map((col, i) => {
              if (!criterios[col]) return '';
              let valorCelula = row[i];
              ...
              if (faixaAplicada) {
                return `
                  <tr>
                    <td class="px-2 py-1">${col}</td>
                    <td class="px-2 py-1">${row[i]}</td>
                    <td class="px-2 py-1 font-semibold">${faixaAplicada.classif}</td>
                    <td class="px-2 py-1 text-center font-bold">${faixaAplicada.nota}</td>
                    <td class="px-2 py-1"><span class="inline-block w-5 h-5 rounded-full border" style="background:${faixaAplicada.cor}"></span></td>
                  </tr>
                `;
              } else {
                return `
                  <tr>
                    <td class="px-2 py-1">${col}</td>
                    <td class="px-2 py-1">${row[i]}</td>
                    <td class="px-2 py-1 text-gray-400 italic">N/A</td>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Adicione esta checagem para garantir que o botão existe antes de adicionar o evento
